...
@view_config(route_name='login', renderer='templates/login.mako', require_csrf=True)
def login(request):
    ...
    password = ''
    now = datetime.now()
    attempts = request.db_sg.login_attempts.find_one({'ip': request.remote_addr})
    ...
    if attempts is not None and attempts['banned_until'] is not None and attempts['banned_until'] > now:
        ...
        if not (username is None or password is None):
            user = request.db_sg.users.find_one({'username': username})
            if user is not None:
                ...
                if is_valid:
                    ...
                    request.db_sg.login_attempts.update_one({'ip': ip},
                                                            {'$set': {'banned_until': None, 'logins': []}})
                    return exc.HTTPFound(location='/admin', headers=headers)
            else:
        ...
        if not is_valid:
            ...
            if attempts is None:
                ...
                request.db_sg.login_attempts.insert_one(attempts)
            else:
                # failed login attempts in the last 10 minutes
                ...
                request.db_sg.login_attempts.update_one({'_id': attempts['_id']}, update)
